...
table(morph_measures$clade_5, morph_measures$Individual_ID)
table(morph_measures$clade_2, morph_measures$Individual_ID)
...
all_summary <- bind_rows(morph_pop_summary,
                         stoma_pop_summary,
                         pollen_pop_summary) %>% 
  select(variable, Individual_ID, N, mean, sd, se, ci) %>% 
