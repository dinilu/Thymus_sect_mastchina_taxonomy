...
morph_pop_minmax <- lapply(morph_vars, FUN = \(x)summarise(group_by(morph_measures, Population_ID),
...
pollen_pop_minmax <- lapply(pollen_vars, FUN = \(x)summarise(group_by(pollen_measures, Population_ID),
                                                               min = min(get(x), na.rm = TRUE),
                                                               max = max(get(x), na.rm = TRUE)))
...
stoma_pop_minmax <- lapply(stoma_vars, FUN = \(x)summarise(group_by(stoma_measures, Population_ID),
                                                             min = min(get(x), na.rm = TRUE),
                                                             max = max(get(x), na.rm = TRUE)))

# Set names in both summaries

names(stoma_pop_summary) <- names(stoma_pop_minmax) <- stoma_vars
